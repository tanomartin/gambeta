...
if (@$_POST["a_edit"] <> "") {
	$torneos->CurrentAction = $_POST["a_edit"]; // Get action code
	GetUploadFiles(); // Get upload files
	LoadFormValues(); // Get form values
} else {
	$torneos->CurrentAction = "I"; // Default action is display
}

// Check if valid key
if ($torneos->id->CurrentValue == "") Page_Terminate($torneos->getReturnUrl()); // Invalid key, exit
switch ($torneos->CurrentAction) {
	case "I": // Get a record to display
		...
		break;
	Case "U": // Update
		$torneos->SendEmail = TRUE; // Send email on update success
		if (EditRow()) { // Update Record based on key
			$_SESSION[EW_SESSION_MESSAGE] = "Actualizacion exitosa"; // Update success
			Page_Terminate($torneos->getReturnUrl()); // Return to caller
		} else {
...
// Render the record
$torneos->RowType = EW_ROWTYPE_EDIT; // Render as edit
RenderRow();
?>
...
<p><span class="phpmaker">Editar TABLA: Torneos<br><br><a href="<?php echo $torneos->getReturnUrl() ?>">Volver</a></span></p>
<?php
...
?>
<form name="ftorneosedit" id="ftorneosedit" action="torneosedit.php" method="post" enctype="multipart/form-data" onSubmit="return ew_ValidateForm(this);">
<p>
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table class="ewTable">
	<input type="hidden" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($torneos->id->CurrentValue) ?>">
	<tr class="ewTableRow">
...
<input type="radio" name="a_logo" id="a_logo" value="1" checked>Mantener&nbsp;
<input type="radio" name="a_logo" id="a_logo" value="2">Remover&nbsp;
<input type="radio" name="a_logo" id="a_logo" value="3">Reemplazar<br>
<?php } else { ?>
<input type="hidden" name="a_logo" id="a_logo" value="3">
<?php } ?>
<input type="file" name="x_logo" id="x_logo" title=""  onchange="if (this.form.a_logo[2]) this.form.a_logo[2].checked=true;"<?php echo $torneos->logo->EditAttributes() ?>>
</span></td>
	</tr>
	<tr class="ewTableRow">
...
function LoadFormValues() {
	...
	// Load from form
	global $objForm, $torneos;
	$torneos->id->setFormValue($objForm->GetValue("x_id"));
...
function RestoreFormValues() {
	global $torneos;
	$torneos->id->CurrentValue = $torneos->id->FormValue;
...
function EditRow() {
	...
	if ($rs->EOF) {
		...
		if ($torneos->logo->Upload->Action == "2" || $torneos->logo->Upload->Action == "3") { // Update/Remove
		$torneos->logo->DbValue = $rs->fields('logo'); // Get original value
		...
		$bUpdateRow = $torneos->Row_Updating($rsold, $rsnew);
		if ($bUpdateRow) {
		...
		// Field logo
			...
			$conn->raiseErrorFn = 'ew_ErrorFn';
			$EditRow = $conn->Execute($torneos->UpdateSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
	...
	if ($EditRow) {
		$torneos->Row_Updated($rsold, $rsnew);
	}
	$rs->Close();
