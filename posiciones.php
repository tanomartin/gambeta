<?	include_once "include/config.inc.php";
	...
	$oObj = new TorneoCat();
	$aCategorias = $oObj->getByTorneo($oTorneo->id_torneo);
	$color = $oTorneo->color;

	$oObj1 = new Pantallas();
	$aPantallas = $oObj1->getByTorneoCat($_POST['id']);
			...
			<div id="imagen" style="float:left; vertical-align:top">
                ...
                <div id="categorias">
                    ...
                    <? for ($i=0; $i<count($aCategorias);$i++) {
						...
						?>
                        	<div style="float:left" class="color_categoria_seleccionada_<?= $color ?>"><?= strtoupper($aCategorias[$i][nombrePagina]) ?></div> 
                        <? } else { ?>
		                       <div style="float:left; cursor:pointer" class="categoria_submenu" onclick="cambiar(<?= $aCategorias[$i][id]?>)"><?= strtoupper($aCategorias[$i][nombrePagina]) ?></div>
						<? }
                  ...
                  <img src="pantallas_fijas/<?= $aPantallas [0]['posiciones'] ?>" />
		        </div> 
			</div>
        </div>
